...
> 15 modules · bilingual (EN / 繁中) · with worked examples. The English skill package lives in [cartographer-en/](./cartographer-en/); scope boundaries in [docs/SCOPE.zh-TW.md](./docs/SCOPE.zh-TW.md).
...
2. **NFR and security dropped** — only features get written; performance/privacy/compliance are absent (even Apple's AirPods PRD did this: collected health data with zero privacy handling)
...
1. Install (see [docs/INSTALL.md](./cartographer-en/docs/INSTALL.md)).
...
Built on the leaked Apple AirPods Pro hardware PRD as a skeleton — porting its strengths (atomic numbered requirements, priorities, measurability, persona-driven, open questions, glossary, competitive analysis) and fixing its weaknesses (added success-metric instrumentation, split assumptions/risks, elevated security, mandatory acceptance criteria, requirement dependencies, a traceability matrix). See [docs/DESIGN.md](./cartographer-en/docs/DESIGN.md).
